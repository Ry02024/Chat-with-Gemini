...
def initialize_app_configs(mode_from_arg=None):
    ...
    # allowed_users_list_str を関数の最初で初期化
    allowed_users_list_str = None
    ...
    if ENV_TYPE == 'local_direct':
        ...
        if ENV_TYPE == 'local_sm_test':
            ...
            # allowed_users_list_str がSMから読み込めなかった場合は None のまま
    else:
    ...
    if allowed_users_list_str:
        ...
        print(f"警告: 許可ユーザーリスト(allowed_users_list_str)が設定/取得されていません。ALLOWED_USERS_LISTは '{ALLOWED_USERS_LIST}' になります。")
